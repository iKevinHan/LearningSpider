...
def main():
    ...
    # 打开页面至屏幕最大尺寸
    driver = webdriver.Chrome()
    driver.get('https://www.binance.co/register.html')
    driver.maximize_window()
    # 获取输入手机号码的表单
    input1 = driver.find_element_by_name('email')
    # 输入注册号码
    input1.send_keys("test@buaa.edu.cn")
    time.sleep(0.3)
    input2 = driver.find_element_by_id('regiterPassword')
    # 输入注册号码
    input2.send_keys("Dev@1234")
    time.sleep(0.3)
    input3 = driver.find_element_by_id('regiterRepeatPassword')
    # 输入注册号码
    input3.send_keys("Dev@1234")
    time.sleep(0.3)
    ...
    # 获取打开滑块验证码页面的元素
    getcheck = driver.find_element_by_id('register-btn')
    # 点击进入滑块验证码页面
    getcheck.click()
    time.sleep(1)
    driver.get_screenshot_as_file("img.png")#对整个页面截图
    imgelement = driver.find_element_by_xpath('/html/body/div[2]/div[2]/div[6]/div/div[1]/div[1]/div/a/div[1]/canvas')  # 定位验证码
    ...
    i = Image.open("img.png")  # 打开截图
    i = i.convert('RGB')
    time.sleep(0.5)
    frame1 = i.crop(rangle)  # 使用Image的crop函数，从截图中再次截取我们需要的区域
    frame1.save('new.png')
    driver.find_element_by_xpath('/html/body/div[2]/div[2]/div[6]/div/div[1]/div[2]/div[2]').click()
    ...
    imgelement = driver.find_element_by_xpath('/html/body/div[2]/div[2]/div[6]/div/div[1]/div[1]/div/a/div[1]/div/canvas[2]')
    ...
    print(loc)
    #找到滑动的圆球
    element=driver.find_element_by_xpath('/html/body/div[2]/div[2]/div[6]/div/div[1]/div[2]/div[2]')
